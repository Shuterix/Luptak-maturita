...
export async function GET(request: NextRequest) {
	try {
		...
		if (!clubId) {
			return NextResponse.json({ error: 'Student not associated with a club' }, { status: 400 })
		}
		...
		return NextResponse.json({ lessons: studentLessons }, { status: 200 })
	} catch (error: any) {
